...
- **Injected Page Script**: `src/injected.ts`. Runs in the page context, talks to the Swiss app runtime via `window.__SWISS_DEVTOOLS_BRIDGE__` and posts results back to the extension.
...
- Zip command: `pnpm -F @swissjs/browser-extension build && pnpm -F @swissjs/browser-extension zip`.
- Artifact name: `browser-extension.zip`.
